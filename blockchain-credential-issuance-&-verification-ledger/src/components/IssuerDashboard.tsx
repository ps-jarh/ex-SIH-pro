...
export const IssuerDashboard: React.FC<IssuerDashboardProps> = ({
  ...
  const handleIssueCredential = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Asymmetric ECDSA digital signature
      const signature = await signPayload(payloadHash, selectedInst.privateKey);
      ...
      // Mine and append block
      const updatedChain = await issueCredentialBlock(chain, [signedCred], `${selectedInst.name} (Validator Node)`);
    ...
    <div id="issuer-dashboard" className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="bg-white/[0.03] border border-white/10 rounded-xl p-6 shadow-xl space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 bg-black/40 p-2.5 rounded-xl border border-white/10 shrink-0">
